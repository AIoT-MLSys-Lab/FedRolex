...
import models
from data import SplitDataset, make_data_loader, GenericDataset
from logger import Logger
from metrics import Metric
from utils import make_optimizer, collate, to_device


@ray.remote(num_gpus=0.85, num_cpus=1)
class ResnetClient:
    ...
    def update(self, client_id, dataset_ref, model_ref):
        ...
        data_split = ray.get(dataset_ref['split'][client_id])
        self.dtl = len(data_split)
        label_split = ray.get(dataset_ref['label_split'])
        local_parameters = model_ref['local_params']
        self.local_parameters = ray.get(local_parameters)
        self.client_id = client_id
        self.model_rate = model_ref['model_rate']
        self.data_loader = make_data_loader({'train': GenericDataset(data_split)})['train']
    ...
    def step(self, m, num_active_users, start_time):
        ...
        if 'resnet18' in cfg['model_name']:
            # print("res18 model found")
            self.model = models.resnet18(model_rate=self.model_rate, cfg=self.cfg).to('cpu')
        elif 'resnet34' in cfg['model_name']:
            # print("res34 model found")
            self.model = models.resnet34(model_rate=self.model_rate, cfg=self.cfg).to('cpu')
        elif 'resnet50' in cfg['model_name']:
            # print("res50 model found")
            self.model = models.resnet50(model_rate=self.model_rate, cfg=self.cfg).to('cpu')
        else:
            # print("No model found")
            self.model = models.resnet101(model_rate=self.model_rate, cfg=self.cfg).to('cpu')
    ...
    def test_model_for_user(self, m, ids):
        cfg = self.cfg
        metric = Metric()
        [data_split, model, label_split] = ray.get(ids)
        model = model.to('cuda')
