...
class ResnetServerRoll:
    ...
    def broadcast(self, lr):
        ...
        return configs, self.param_idx, self.user_idx
    ...
    def combine(self, local_parameters, param_idx, user_idx):
        ...
        for k, v in updated_parameters.items():
            parameter_type = k.split('.')[-1]
            count[k] = v.new_zeros(v.size(), dtype=torch.float32).cpu()
            tmp_v = v.new_zeros(v.size(), dtype=torch.float32).cpu()
